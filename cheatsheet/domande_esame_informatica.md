...
Per costruire l'applicazione web usiamo Flask, che è un framework Python leggero e flessibile. Flask ci permette di organizzare il codice in Blueprint, che sono come "moduli" dove raggruppiamo route correlate - per esempio tutte le route per gestire gli utenti in un blueprint, quelle per i prodotti in un altro. Dentro il progetto Flask ci sono alcuni file importanti: init.py trasforma una cartella in un package Python e contiene la "fabbrica" che crea l'applicazione, mentre la cartella instance è dove Flask salva cose specifiche di quella installazione - tipo il file .sqlite che contiene il nostro database SQLite.
